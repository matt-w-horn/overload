...
declarations under `root`, sorted by name. Membership is
`OverloadTest.auditRule` — the axiom sweep's rule, private declarations
un-mangled and included — minus the auto-generated and projection
exemptions, so the manifest universe is the coverage gate's universe. -/
def manifestEntries (root : Name) : CoreM (Array Entry) := do
  let env ← getEnv
  let mut names : Array Name := #[]
  for (n, _) in env.constants.toList do
    if OverloadTest.auditRule root n
