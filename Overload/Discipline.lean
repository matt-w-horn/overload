...
Two instances close the loop with `Overload/Priority.lean`, whose two
regimes turn out to be two disciplines:
...
strict-priority allocation of `Overload/Priority.lean`. -/
...
`sharedGoodput` regime of `Overload/Priority.lean`, reframed as an
