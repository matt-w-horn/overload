...
verdict on every pair before its verdicts on real pairs are accepted: the
defective pairs must come back with the keyed verdict, the known-good
...
- **pair6–pair8** — known-good pairs from documented Mathlib lemmas
  (statements verified against the local pin); calibrate the
...
- **pair14** — dependency-bearing: the dispatcher supplies the `deps`
  entries as the per-pair block's verified dependency docstrings, and the
...
`docstring`, and (where present) `deps` — `expected`, `axis`, and `why`
are the answer key and never leave this directory. Names are styled like
real declarations so a referee cannot recognize a calibration run.
Statements are over plain Mathlib vocabulary, probe-able through
`scripts/claim-probe.sh`; every mathematical claim in a `why` field was
probe-verified when the pair was written.
