...
- `Amplification.lean`: the truncated-geometric expected attempt count
  `expAttempts`, defined as the finite geometric sum so it is total; the
  `(1-p^n)/(1-p)` quotient is a lemma.
...
- `Deadline.lean`: how many attempts fit a deadline (`neff`, via
  `Nat.findGreatest`); the re-armed vs remaining-deadline distinction and
  the constant, linear, and geometric backoff levers.
