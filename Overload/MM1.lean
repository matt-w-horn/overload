...
stance `Overload/Spec.lean` takes for PASTA. Nothing here claims
discipline-invariance, and the M/M/1 *retrial* queue's closed forms (test 10
in `Spec.lean`) remain unformalized.
...
`Overload/Spec.lean` takes for PASTA) rather than deriving it. Total; the
...
cancellation needs. Nothing ergodic is established here. `Overload/Little.lean`
proves the deterministic sample-path law; identifying its time averages with
