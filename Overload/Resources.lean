...
`coupledLoop`'s docstring in `Overload/CoupledStack.lean`). A componentwise
vector `CongestedEq` is deliberately not attempted here — per-resource
scalar inequalities are the honest current form.
-/
