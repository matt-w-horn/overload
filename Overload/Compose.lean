...
reference presentation, kept as stated. `Overload/CoupledStack.lean` consumes
the one-layer `composeFail` (with its `Icc` and monotonicity lemmas),
building its own recursions over own-cap pairs `(ℓᵢ, nᵢ)` (the power applied
on invocation exit) — the shape that makes the amplification inductions
clean. The `stackFail` fold itself has no downstream consumers and stays as
