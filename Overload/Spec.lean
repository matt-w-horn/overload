...
* Test 8 (truncated-geometric histogram under exogenous `p`) —
  `sum_attemptWeight` / `sum_mul_attemptWeight` (`Overload/Amplification.lean`).
* Test 9 (goodput plateau under zero waste) — `plateau`
  (`Overload/Plateau.lean`).
...
joint matrix) rather than an untracked loss — `Overload/Observability.lean`
proves why instruments built on the wire label alone cannot see it.
-/
...
`G ≤ min(λ, C/s̄)` — for every retry scheme, backoff policy, and topology,
